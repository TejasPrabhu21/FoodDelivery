...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchEmail = async () => {
      try {
        ...
        handleError("Error retrieving savemail");
      }
    };
    fetchEmail();
  }, []);
  ...
  const fetchUserId = async (savemail: string) => {
    ...
    if (error) {
      ...
      handleError("Error fetching user id");
      return null;
    }
  ...
  const fetchOrdersByUserId = async (userId: number) => {
    const { data, error } = await supabase
      .from("Order")
      .select(
        "Order_id, Order_date, Order_Status, Address, Total_Amount, Delivery_Fee"
      )
      .eq("User_id", userId)
  ...
  const fetchOrderItems = async (orderId: number) => {
    ...
    if (error) {
      ...
      handleError("Error fetching order items");
      return [];
    }
  ...
  const renderOrderItem = ({ item }: { item: Order }) => (
    <View
      className={`shadow-md rounded-lg ${getOrderBorderColor(item.Order_Status)} border-2 p-4 ml-5 mr-5 mb-2 bg-white`}
    >
      ...
      <Text className="font-bold text-lg mt-2">
        Total: ₹{item.Total_Amount + item.Delivery_Fee}
      </Text>
      <TouchableOpacity
        className="mt-2 bg-[#ca681c] p-3 rounded flex-row items-center"
      ...
      <Modal visible={showModal} transparent={true} animationType="slide">
        <View className="flex-1 bg-black/50 justify-center">
          <View className="bg-white mx-5 rounded-lg p-5">
            ...
            <TouchableOpacity
              className="mt-4 bg-[#ca681c] p-3 rounded items-center"
